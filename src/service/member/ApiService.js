...
export function signin(memberDTO) {
  return call("/auth/login", "POST", memberDTO).then((response) => {
    ...
    //완빈 admin role 구분
    if (response.token) {
      const token = response.token; // 토큰 저장
      localStorage.setItem("ACCESS_TOKEN", token); //브라우저 저장소에 토큰을 저장
      //API 호출 시 해당 토큰을 사용할 수 있도록 브라우저 저장소에 토큰을 저장

      //토큰은 3부분으로 구성되어 role과 id와 같은 정보가 들어 있는
      //payload(가운데 부분)을 가져온다
      const payloadBase64 = token.split(".")[1];
      //payloadBase64는 Base64로 인코딩되어 해독해야한다
      const payloadJson = atob(payloadBase64);
      //atob 함수는 Base64 문자열을 사람이 읽을 수 있도록 텍스트로 바꿔준다.

      //위에서 얻은 xprtmxm(JSON 문자열)를 자바스크립트 객체로 바꿔준다.
      // payload.role 같은 방식으로 정보를 꺼낼 수 있음
